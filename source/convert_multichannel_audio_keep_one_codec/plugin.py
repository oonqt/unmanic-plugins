...
from convert_multichannel_audio_keep_one_codec.lib.ffmpeg import Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.convert_multichannel_audio_keep_one_codec")
...
class Settings(PluginSettings):
    ...
    def __set_keep_codec_form_settings(self):
        values = {
            "label":      "Select Codec",
            "description":    "Select a multichannel codec to keep - any streams encoded with this are left as is, others are converted to designated encoder.  Select None to convert all",
            "input_type": "select",
            "select_options": [
                {
                    "value": "none",
...
def on_library_management_file_test(data):
    ...
    if stream_to_encode:
        ...
        logger.info("do not add file '{}' to task list - no 6 (or more) multichannel audio streams found".format(abspath))
