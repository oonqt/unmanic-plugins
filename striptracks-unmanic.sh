...
# CUSTOM VERSION OF STRIPTRACKS TO TRIGGER UNMANIC PROCESSING AFTER STRIPTRACKS PROCESSING TO PREVENT SIMULTANEOUS PROCESSING OF THE SAME FILE. 
# PLEASE SEE CONFIGURATION ON LINE #56 TO CONFIGURE UNMANIC. 
# KEEP THE STRIPTRACKS DOCKERMOD INSTALLED AS IT INSTALLS DEPENDENCIES NEEDED, BUT PLACE THIS SCRIPT IN A DIRECTORY MOUNTED TO YOUR CONTAINER AND REFERENCE THIS SCRIPT INSTEAD OF THE STRIPTRACKS ONE IN /USR/BIN
...
function main {
  ...
  set_title_and_exit_if_nothing_removed # Unmanic trigger interrupt
  remux_video
  set_perms_and_owner
...
Options and Arguments:
  -a, --audio <audio_languages>    Audio languages to keep
                         ...
                         eo
  rescan_and_cleanup          plus \`+\` and one or more modifiers.
...
function end_script {
  ...
  # Trigger unmanic here if wasn't renamed
